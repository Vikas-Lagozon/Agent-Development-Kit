...
async def add_expense_tool(date: str, amount: float, category: str, subcategory: str = '', note: str = '') -> Dict[str, Any]:
    ...
    await _ensure_db()
    if amount <= 0: return {"status": "error", "error": "Amount must be > 0"}
    async with aiosqlite.connect(DB_PATH) as db:
...
async def edit_expense_tool(expense_id: int, date: Optional[str] = None, amount: Optional[float] = None, 
    ...
    if not updates: return {"status": "error", "message": "No fields provided for update"}
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
        if cursor.rowcount == 0: return {"status": "error", "message": "Expense ID not found"}
