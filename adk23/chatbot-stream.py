...
async def chat_stream(user_input: str, session_id: str):
    """
    Streams the chatbot response in real-time.
    Yields individual text chunks as they are generated by the model.
    """
    ...
    async for event in events:
        # Get the content object from the event
        ...
        # Check if this event contains a partial (streaming) response or final text
        # We yield whenever there is text in the parts list
        for part in parts:
            text_chunk = getattr(part, "text", None)
            if text_chunk:
                yield text_chunk

        # Note: 'event.partial' is True while the LLM is still typing.
        # When 'event.is_final_response()' is True, the entire turn sequence is complete.
