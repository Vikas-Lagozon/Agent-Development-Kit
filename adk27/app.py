...
async def health_check():
    ...
    # ── 2. Ping all A2A agents concurrently ───────────────────────────────
    a2a_results = await asyncio.gather(
        _check_a2a_agent("hello_world_agent", hello_world_agent),
        return_exceptions=False,
    )
    ...
    # ── 4. Overall status ─────────────────────────────────────────────────
    all_results = list(mcp_results) + list(a2a_results)
