...
async def health_check():
    ...
    Pings all four MCP servers and reports their status.
    ...
    mcp_results = await asyncio.gather(
        _check_mcp("web_reader",      web_reader_mcp),
    ...
    # ── 3. Overall status ─────────────────────────────────────────────────
    all_ok      = all(r["status"] == "ok" for r in mcp_results)
    any_ok      = any(r["status"] == "ok" for r in mcp_results)
